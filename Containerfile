...
RUN xargs -a removals rpm-ostree override remove \
    && ostree container commit \
    && rm removals
...
RUN xargs -a layers rpm-ostree install -y \
    && ostree container commit \
    && rm layers
...
RUN systemctl enable rpm-ostree{d-automatic,-countme}.timer flatpak-system-update.timer
